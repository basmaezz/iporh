...
class Project extends Model
{
    ...
    {
        return $this->belongsToMany(Image::class, 'image_categories', 'owner_id', 'image_id')->where('owner_model', 'App\Project');
    }

    public function updateImages($images)
    {
        if (isset($images) && count($images) > 0) {
            $pivotData = array_fill(0, count($images), ['owner_model' => 'App\Project']);
