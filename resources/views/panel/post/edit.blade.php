@php
    $breadcrumb_array = [];
    array_push($breadcrumb_array,collect(['is_last'=>true,'name'=>$post->title,'link'=> '#']));
    array_push($breadcrumb_array,collect(['is_last'=>false,'name'=>'الأخبار','link'=> lang_route('panel.post.all')]));
@endphp

@extends('panel.layouts.index',['sub_title'=>'تعديل الخبر' ,'breadcrumb_array'=> $breadcrumb_array])
@section('main')
    @push('panel_css')
    ...
    <div class="content">
        {!! Form::open(['id'=>'form','method'=>'PUT','url'=>lang_route('panel.post.edit',['id'=>$post->id]),'to'=>lang_route('panel.post.all')]) !!}
                ...
                <div class="card">
                    <div class="card-body">
                        <fieldset class="form-group">
                            <label>العنوان</label>
                            <input class="form-control"  type="text" name="title" placeholder="الرجاء إدخال العنوان"  value="{{$post->title}}" required>
                        </fieldset>

                        <fieldset class="form-group">
                            <label>الوصف</label>
                            <textarea class="form-control " rows="4" type="text" name="description" placeholder="الرجاء إدخال الوصف" required>{!! $post->description !!}</textarea>
                        </fieldset>
                            ...
                            <select class="form-control" name="category_id" data-placeholder="إختيار التصنيف" required>
                                ...
                                @if(isset($items) && $items->count() > 0)
                                    @foreach($items as $item)
                                        <option value="{{$item->id}}" {{is_selected($item->id,$post->category_id)}}>{{get_text_locale($item,'text')}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </fieldset>
                        ...
                        <fieldset class="form-group">
                            ...
                            <input class="form-control"  type="url" name="video" value="{{$post->video}}" placeholder="الرجاء إدخال رابط الفيديو" >
                        </fieldset>

                        <label>رفع الصور</label>
                        <fieldset class="form-group">
                            <div id="app">
                                <dropzone :images_data="{{json_encode($post->images)}}"></dropzone>
                            </div>
                        </fieldset>
                    </div>
                </div>
                            ...
                            <form id="single" action="{{csrf_token()}}">
                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                    ...
                                    <div class="fileinput-new thumbnail"
                                        ...
                                        <img src="{{image_url($post->image,'300x300')}}"
