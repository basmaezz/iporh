@php
    $breadcrumb_array = [];
    array_push($breadcrumb_array,collect(['is_last'=>true,'name'=>'إضافة خدمة','link'=> '#']));
    array_push($breadcrumb_array,collect(['is_last'=>false,'name'=>'الخدمات','link'=> lang_route('panel.service.all')]));
@endphp

@extends('panel.layouts.index',['sub_title'=>'إضافة خدمة جديد' ,'breadcrumb_array'=> $breadcrumb_array])
@section('main')
    @push('panel_css')
    ...
    <div class="content">
        {!! Form::open(['id'=>'form','method'=>'POST','url'=>lang_route('panel.service.create'),'to'=>lang_route('panel.service.all')]) !!}
