@php
    $breadcrumb_array = [];
    array_push($breadcrumb_array,collect(['is_last'=>true,'name'=>'إضافة إعلان','link'=> '#']));
    array_push($breadcrumb_array,collect(['is_last'=>false,'name'=>'الإعلانات','link'=> lang_route('panel.advertisement.all')]));
@endphp

@extends('panel.layouts.index',['sub_title'=>'إضافة إعلان جديد' ,'breadcrumb_array'=> $breadcrumb_array])
@section('main')
    @push('panel_css')
    ...
    <div class="content">
        {!! Form::open(['id'=>'form','method'=>'POST','url'=>lang_route('panel.advertisement.create'),'to'=>lang_route('panel.advertisement.all')]) !!}
