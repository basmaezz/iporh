@php
    $breadcrumb_array = [];
    array_push($breadcrumb_array,collect(['is_last'=>true,'name'=>$advertisement->title,'link'=> '#']));
    array_push($breadcrumb_array,collect(['is_last'=>false,'name'=>'الإعلانات','link'=> lang_route('panel.advertisement.all')]));
@endphp

@extends('panel.layouts.index',['sub_title'=>'تعديل الإعلان' ,'breadcrumb_array'=> $breadcrumb_array])
@section('main')
    @push('panel_css')
    ...
    <div class="content">
        {!! Form::open(['id'=>'form','method'=>'PUT','url'=>lang_route('panel.advertisement.edit',['id'=>$advertisement->id]),'to'=>lang_route('panel.advertisement.all')]) !!}

        <div class="row">
            <div class="col-md-8">
                <input type="hidden" id="photo" name="image" value="{{$advertisement->image}}">
                <div class="card">
                    <div class="card-body">
                        <fieldset class="form-group">
                            <label>العنوان</label>
                            <input class="form-control"  type="text" name="title" placeholder="الرجاء إدخال العنوان"  value="{{$advertisement->title}}" required>
                        </fieldset>
                            ...
                            <form id="single" action="{{csrf_token()}}">
                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                    ...
                                    <div class="fileinput-new thumbnail"
                                        ...
                                        <img src="{{image_url($advertisement->image,'300x300')}}"
