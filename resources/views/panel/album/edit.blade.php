@php
    $breadcrumb_array = [];
    array_push($breadcrumb_array,collect(['is_last'=>true,'name'=>$album->title,'link'=> '#']));
    array_push($breadcrumb_array,collect(['is_last'=>false,'name'=>'الألبومات','link'=> lang_route('panel.album.all')]));
@endphp

@extends('panel.layouts.index',['sub_title'=>'تعديل الألبوم' ,'breadcrumb_array'=> $breadcrumb_array])
@section('main')
    @push('panel_css')
                ...
                <div class="card">
                    <div class="card-body">
                        <fieldset class="form-group">
                            <label>العنوان</label>
                            <input class="form-control"  type="text" name="title" placeholder="الرجاء إدخال العنوان"  value="{{$album->title}}" required>
                        </fieldset>

                        <fieldset class="form-group">
                            <label>الوصف</label>
                            <textarea class="form-control " rows="4" type="text" name="description" placeholder="الرجاء إدخال الوصف" required>{!! $album->description !!}</textarea>
                        </fieldset>
                            ...
                            <select class="form-control" name="category_id" data-placeholder="إختيار التصنيف" required>
                                ...
                                @if(isset($items) && $items->count() > 0)
                                    @foreach($items as $item)
                                        <option value="{{$item->id}}" {{is_selected($item->id,$album->category_id)}}>{{get_text_locale($item,'text')}}</option>
                                    @endforeach
                                @endif
                            </select>
                        </fieldset>
