@php
    $breadcrumb_array = [];
    array_push($breadcrumb_array,collect(['is_last'=>true,'name'=>$project->title,'link'=> '#']));
    array_push($breadcrumb_array,collect(['is_last'=>false,'name'=>'المشاريع','link'=> lang_route('panel.project.all')]));
@endphp

@extends('panel.layouts.index',['sub_title'=>'تعديل المشروع' ,'breadcrumb_array'=> $breadcrumb_array])
@section('main')
    @push('panel_css')
    ...
    <div class="content">
        {!! Form::open(['id'=>'form','method'=>'PUT','url'=>lang_route('panel.project.edit',['id'=>$project->id]),'to'=>lang_route('panel.project.all')]) !!}

        <div class="row">
            <div class="col-md-8">
                <input type="hidden" id="photo" name="image" value="{{$project->image}}">
                <div class="card">
                    <div class="card-body">
                        <fieldset class="form-group">
                            <label>العنوان</label>
                            <input class="form-control"  type="text" name="title" placeholder="الرجاء إدخال العنوان"  value="{{$project->title}}" required>
                        </fieldset>
                        ...
                        <fieldset class="form-group">
                            ...
                            <input class="form-control"  type="url" name="video" value="{{$project->video}}" placeholder="الرجاء إدخال رابط الفيديو" >
                        </fieldset>

                        <label>رفع الصور</label>
                        <fieldset class="form-group">
                            <div id="app">
                                <dropzone :images_data="{{json_encode($project->images)}}"></dropzone>
                            </div>
                        </fieldset>
                    </div>
                </div>
                            ...
                            <form id="single" action="{{csrf_token()}}">
                                <div class="fileinput fileinput-new" data-provides="fileinput">
                                    ...
                                    <div class="fileinput-new thumbnail"
                                        ...
                                        <img src="{{image_url($project->image,'300x300')}}"
